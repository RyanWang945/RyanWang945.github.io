...
tags:
  ...
  - controller
  - performance
description: "重构在阿里云负责的 registry secret controller，聊聊延迟队列、事件优先级与 Controller 解耦，再跑一组固定参数、50 到 5000 个 NS 的压测。"
---
...
![NS 数量与整轮 Secret 刷新耗时](/images/posts/registry-secret-controller-refactor/01-refresh-scaling.png)

5000 份 Secret 刷新约 46.31 秒。刷新期间再新建一个 NS，也是两个 SA，Secret 约 0.157 秒就绪，两个 SA 约 0.330 秒全部 patch 完成。这两个时间都从 NS 创建成功起算。

![刷新期间新 NS 的 Secret 创建耗时](/images/posts/registry-secret-controller-refactor/02-new-namespace-secret.png)

![刷新期间新 NS 的两个 SA 完成耗时](/images/posts/registry-secret-controller-refactor/03-new-namespace-serviceaccounts.png)

这轮 VM 内存实际是 4/3/3 GiB，鉴权和调度时钟用了 mock，K8s 资源操作实跑。[实验口径与原始数据](https://github.com/RyanWang945/kubernetes-registry-secret-controller/blob/b4667a3/docs/benchmarks/2026-09-06/methodology.md)单独放了一份。

## 5. 总结

写出一个能工作的 Controller 并不难，真正困难的是它在资源规模扩大两个数量级之后仍然能工作。10 个 Namespace 下看不到的问题，在 5000 个 Namespace 下会变成事件放大、队头阻塞、API Server 请求预算和刷新窗口的容量问题。对 Controller 来说，正确性只是起点，调度、公平性、幂等和容量规划才决定它能不能真正跑在生产环境里。
